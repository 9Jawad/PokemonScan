...
int main(int argc, char* argv[]) {
    ...
    const char* banqueImg = NULL;
    if (argc == 3){
        banqueImg = argv[2];
    }
    const char* fileToCompare = argv[1];  // Chemin de l'image à comparer
    ...
    if (child1 == 0) {
        ...
        while (!finished){
            ...
            if (nbytes_read < 0) {
                ...
                if (cheminFileName != NULL) {
                    ...
                    char command[256];
                    snprintf(command, sizeof(command), "img-dist %s %s", fileToCompare, cheminFileName);
        ...
        if (child2 == 0) {
            ...
            // Diviser le texte en lignes et écrire chaque ligne dans le pipe
            char* copie_texte = strdup(banqueImg);  // Pour ne pas modifier la chaine originale
            char* fileName = strtok(copie_texte, "\n");  // Divise le texte en lignes
            while (fileName != NULL) {
            ...
            // Verifie si les processus fils se sont terminés correctement
            if (WIFEXITED(status1)) {
                // int exitStatus1 = WEXITSTATUS(status1);
                // printf("exitStatus1 = %d \n", exitStatus1); // Test
            } else {
                printf("child1 ne s'est pas terminé correctement\n");  // Cas ou le fils2 termine avant le fils1
                perror("child1");
                // Le child1 ne s'est pas terminé correctement
            }

            if (WIFEXITED(status2)) {
                // int exitStatus2 = WEXITSTATUS(status2);
                // printf("exitStatus2 = %d \n", exitStatus2); // Test
            } else {
                printf("child2 ne s'est pas terminé correctement\n");  // Cas ou le fils1 termine avant le fils2
                perror("child2");
                // Le child2 ne s'est pas terminé correctement
            }
            ...
            if (sharedMemory->minDistance == INT_MAX) {
                ...
                printf("Most similar image found: 'img/%s' with a distance of %d.\n", sharedMemory->minFileName, sharedMemory->minDistance);
            }
        }
    }
